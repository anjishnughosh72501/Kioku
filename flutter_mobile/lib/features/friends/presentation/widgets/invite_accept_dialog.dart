...
class InviteAcceptDialog extends ConsumerStatefulWidget {
  const InviteAcceptDialog({super.key, required this.inviteCode});

  final String inviteCode;

  static Future<void> show(BuildContext context, String inviteCode) {
    return showKiokuDialog<void>(
      context: context,
...
class _InviteAcceptDialogState extends ConsumerState<InviteAcceptDialog> {
  ...
  Future<void> _loadInvite() async {
    ...
    final res = await UserProfileService.instance.resolveInvite(
      widget.inviteCode,
    );
    if (!mounted) return;

    final myCode = UserProfileService.instance.friendCode.trim().toUpperCase();
    if (res.friendCode != null &&
        res.friendCode!.trim().toUpperCase() == myCode) {
      setState(() {
        _loading = false;
        _resolution = res;
    ...
    final currentFriends = await UserProfileService.instance
        .getConnectedFriends();
    if (res.friendCode != null &&
        currentFriends.contains(res.friendCode!.trim().toUpperCase())) {
      setState(() {
        _loading = false;
        _resolution = res;
        _errorMessage =
            'You and ${res.username ?? 'this user'} are already connected friends!';
      });
      return;
    }
  ...
  Future<void> _acceptInvite() async {
    ...
    try {
      final confirmRes = await UserProfileService.instance.confirmInvite(
        inviteCode,
      );
      if (!mounted) return;
      ...
      if (confirmRes != null && confirmRes['status'] == 'already_friends') {
        ...
        showKiokuSnackBar(
          context,
          'Invite confirmed with $name! Friend request received.',
          aboveBottomNav: true,
        );
      }
    } catch (e) {
      ...
      showKiokuSnackBar(
        context,
        e is FriendException
            ? e.message
            : "Couldn't confirm invite. Please try again.",
        aboveBottomNav: true,
      );
    }
  }
